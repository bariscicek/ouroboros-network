...
mkApplicationHandleBundle muxBundle scheduleStopVarBundle awaitVarBundle =
    Bundle
      (mkApplication TokHotApp)
      (mkApplication TokWarmApp)
      (mkApplication TokEstablishedApp)
  where
    mkApplication :: TokAppKind appKind
                  -> WithAppKind appKind (ApplicationHandle muxMode bytes m a b)
    mkApplication tok =
      let app =
            ApplicationHandle {
      ...
      in case tok of
          TokHotApp -> WithHot app
          TokWarmApp -> WithWarm app
          TokEstablishedApp -> WithEstablished app
  ...
  where
        ...
        $ \peerStatusVar -> do
          ...
          case muxPromise of
            MuxRunning connectionId
              ...
              atomically $ do
                writeTVar (projectBundle TokHotApp         scheduleStopVarBundle) Terminate
                writeTVar (projectBundle TokWarmApp        scheduleStopVarBundle) Continue
                writeTVar (projectBundle TokEstablishedApp scheduleStopVarBundle) Continue
              ...
              startProtocols TokWarmApp connHandle
              startProtocols TokEstablishedApp connHandle
    ...
    activatePeerConnection
      ...
      do
        ...
        atomically $ do
          writeTVar (getControlVar TokHotApp pchAppHandles) Continue
          writeTVar (getControlVar TokWarmApp pchAppHandles) Quiesce
          e <- readTVar (getControlVar TokEstablishedApp pchAppHandles)
          evaluate $ assert (e == Continue) ()

        -- start hot peer protocols
        startProtocols TokHotApp connHandle
    ...
    deactivatePeerConnection
      ...
      do
        atomically $ do
          writeTVar (getControlVar TokHotApp pchAppHandles) Terminate

          w <- readTVar (getControlVar TokWarmApp pchAppHandles)
          evaluate $ assert (w == Quiesce) ()
          writeTVar (getControlVar TokWarmApp pchAppHandles) Continue
        ...
        res <-
          timeout spsDeactivateTimeout
                  (atomically $ join $ readTVar (getAwaitVar TokHotApp pchAppHandles))
        case res of
          Nothing -> do
            Mux.stopMux pchMux
            traceWith spsTracer (PeerStatusChangeFailure
    ...
    closePeerConnection
      ...
      do
        atomically $ do
          writeTVar (getControlVar TokWarmApp pchAppHandles) Terminate
          writeTVar (getControlVar TokEstablishedApp pchAppHandles) Terminate
          -- TODO: double check that there is no HOT -> COLD transition
          h <- readTVar (getControlVar TokHotApp pchAppHandles)
        ...
        res <-
          timeout spsCloseConnectionTimeout
                  (atomically $
                    (\a b c -> a <> b <> c)
                      <$> (join $ readTVar (getAwaitVar TokHotApp pchAppHandles))
                      <*> (join $ readTVar (getAwaitVar TokWarmApp pchAppHandles))
                      <*> (join $ readTVar (getAwaitVar TokEstablishedApp pchAppHandles)))
        case res of
          Nothing -> do
            Mux.stopMux pchMux
            traceWith spsTracer (PeerStatusChangeFailure
...
--
startProtocols :: forall (muxMode :: MuxMode) (appKind :: AppKind) peerAddr m a b.
                  ( MonadAsync m
                  , MonadCatch m
                  , HasInitiator muxMode ~ True
                  )
