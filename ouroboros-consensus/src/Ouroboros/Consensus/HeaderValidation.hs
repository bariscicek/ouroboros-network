...
data instance Ticked (HeaderState blk) = TickedHeaderState {
    ...
    , tickedHeaderStateTips      :: StrictSeq (AnnTip blk)
    , tickedHeaderStateAnchor    :: WithOrigin (AnnTip blk)
    }
...
tickHeaderState cfg ledgerView slot HeaderState{..} = TickedHeaderState {
      tickedHeaderStateTips      = headerStateTips
    , tickedHeaderStateAnchor    = headerStateAnchor
...
headerStateTip TickedHeaderState{..} =
    case tickedHeaderStateTips of
      Empty     -> tickedHeaderStateAnchor
      _ :|> tip -> NotOrigin tip
