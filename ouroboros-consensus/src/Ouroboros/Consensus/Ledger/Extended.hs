...
_lemma_protocolLedgerView_applyLedgerBlock
  :: forall blk. (
  ...
  -> LedgerState blk
  -> Either String ()
_lemma_protocolLedgerView_applyLedgerBlock cfg blk st
  ...
  where
    ...
    tickedLedger :: Ticked (LedgerState blk)
    tickedLedger = applyChainTick lcfg (blockSlot blk) st

    lhs :: Except (LedgerErr (LedgerState blk)) (LedgerView (BlockProtocol blk))
    lhs = protocolLedgerView lcfg <$> applyLedgerBlock cfg blk tickedLedger

    rhs :: LedgerView (BlockProtocol blk)
    rhs = forgetTicked $ protocolTickedView lcfg tickedLedger
-}
...
instance ( IsLedger (LedgerState  blk)
  ...
  applyChainTick cfg slot (ExtLedgerState ledger header) =
    ...
    where
      ...
      tickedLedgerView :: Ticked (LedgerView (BlockProtocol blk))
      tickedLedgerView = protocolTickedView lcfg tickedLedgerState
